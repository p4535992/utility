...
public class Sesame2Utilities {
    ...
    public void toRDFFormat(String urlFile, String inputFormat, String outputFormat) {
        try {
            if (StringUtils.isBlank(inputFormat)) inputFormat = "n3";
            // open our input document
            URL documentUrl;
            RDFFormat format;
            InputStream inputStream;
            if (UrlValidator.getInstance().isValid(urlFile)) {
                ...
                urlFile = toStringUriWithPrefix(urlFile);
    ...
    public RDFFormat toRDFFormat(File filePath) {
        ...
        String supportName;
        RDFFormat rdfFormat;
        if(ext.toLowerCase().contains("turtle")) supportName = FilenameUtils.removeExtension(filePath.getAbsolutePath())+".ttl";
    ...
    public long importIntoRepositoryFileChunked(File file,RepositoryConnection mRepositoryConnection) {
        ...
        try {
            ...
            RDFFormat format = toRDFFormat(FilenameUtils.getExtension(file.getAbsolutePath()));
            if (format == null) {
            ...
            if (!format.equals(RDFFormat.NQUADS) && !format.equals(RDFFormat.TRIG) && !format.equals(RDFFormat.TRIX)) {
                if(UrlValidator.getInstance().isValid(CONTEXT)) {
     ...
     */
    public RepositoryManager connectToRepository(String repositoryLocation, String repositoryID) throws MalformedURLException {
        closeRepository();
     ...
     */
    public RepositoryManager connectToRepository(URL repositoryLocation, String repositoryID) throws URISyntaxException {
        closeRepository();
        connectToLocation(repositoryLocation);
        openRepository(repositoryID);
        return mRepositoryManager;
     ...
     * @throws MalformedURLException 
     */
    public RepositoryManager connectToRepository(File repositoryLocation, String repositoryID) throws MalformedURLException {
        closeRepository();
        connectToLocation(repositoryLocation);
        openRepository(repositoryID);
        return mRepositoryManager;
     ...
     * @throws MalformedURLException 
     */
    public RepositoryManager connectToLocation(String urlOrDirectory) throws MalformedURLException {
        logger.info("Calling with String: " + urlOrDirectory);
        if (UrlValidator.getInstance().isValid(urlOrDirectory)) {
     ...
     * @throws URISyntaxException 
     */
    public RepositoryManager connectToLocation(URL urlOrDirectory) throws URISyntaxException {
        //logger.info("Calling with URL: " + urlOrDirectory);
        if (UrlValidator.getInstance().isValid(urlOrDirectory.toString())) {
            connectToRemoteLocation(urlOrDirectory.toString());
            return mRepositoryManager;
        } else if (new File(urlOrDirectory.toURI()).exists()) {
            connectToLocalLocation(urlOrDirectory);
            return mRepositoryManager;
        } else {
            logger.warn("Not exists the url or the File with path:" + urlOrDirectory);
     ...
     * @throws MalformedURLException 
     */
    public RepositoryManager connectToLocation(File urlOrDirectory) throws MalformedURLException {
     ...
     * @throws MalformedURLException 
     */
    private RepositoryManager connectToLocalLocation(String directory) throws MalformedURLException {
     ...
     * @throws MalformedURLException 
     */
    private RepositoryManager connectToLocalLocation(File directory) throws MalformedURLException {
        return connectToLocalLocation(directory.toURI().toURL());
    }
            ...
            try {
                ...
                try {
                    if (inputFormat == null) {
                        ...
                        if(new File(path).exists()){
                            sesameFormat = toRDFFormat(FilenameUtils.getExtension(path));
                        }else {
                ...
                if (!StringUtil.isBlank(baseURI)) {
                    ...
                    if (fileOrDirectory.isDirectory()) {
                        files = (List<File>) FileUtils.listFiles(fileOrDirectory, TrueFileFilter.INSTANCE, TrueFileFilter.INSTANCE);
                    } else {
                    ...
                    Iterable<? extends Statement> iter = toIterable(objectToImport);
     ...
     * @return the OpenRDF Literal Object.
     */
    public Literal toLiteral(Object literalObject, String languageOrDataType) throws URISyntaxException {
        if (UrlValidator.getInstance().isValid(languageOrDataType)) {
            return createLiteralBase(literalObject, new java.net.URI(languageOrDataType));
        } else {
        ...
        switch (arrayLiteralObject.length) {
            ...
            case 2:
                ...
                if (arrayLiteralObject[1] instanceof String) { //Language
                    if (UrlValidator.getInstance().isValid(String.valueOf(arrayLiteralObject[1]))) {
    ...
    public Value toValue(Object resourceOrLiteral) {
        ...
        if (resourceOrLiteral instanceof String) {
            if (UrlValidator.getInstance().isValid(String.valueOf(resourceOrLiteral))) return toURI(resourceOrLiteral);
    ...
    public static <T> List<T> toList(Object collection){
        if(collection instanceof Collection || collection instanceof Map) {
            ...
            if(collection instanceof Object[])return new ArrayList<>(Arrays.asList((T[])collection));
            if(collection instanceof Iterator){
                List<T> list = new ArrayList<>();
    ...
    public static <T> Set<T> toSet(Object collection){
        if(collection instanceof Collection || collection instanceof Map) {
            try {
                ...
                else if (collection instanceof List) return new HashSet((List<T>) collection);
                else if (collection instanceof java.lang.reflect.Array) return new HashSet(Arrays.asList((T[]) collection));
