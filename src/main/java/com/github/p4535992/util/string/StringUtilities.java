...
public class StringUtilities {
    ...
    public static boolean isEmail(String email) {
        return email != null && RegexUtilities.EMAIL_ADDRESS.matcher(email).matches();
    }
    ...
    public static boolean isPhoneNumber(String number) {
        return number != null && RegexUtilities.PHONE.matcher(number).matches();
...
*/
    ...
    public static boolean isURLWithProtocol(String url) {
        return isURL(url) && (RegexUtilities.Protocol_URL.matcher(url).matches() || url.matches("^(https?|ftp)://.*$"));
    }
    ...
    public static boolean isURLWithoutProtocol(String url) {
        return isURL(url) &&
                (RegexUtilities.WEB_URL_NO_PROTOCOL.matcher(url).matches() || RegexUtilities.WEB_URL_NO_PROTOCOL.matcher(url).matches() &&
                 !(RegexUtilities.Protocol_URL.matcher(url).matches() || url.matches("^(https?|ftp)://.*$")));
    }
    ...
    public static boolean isURL(Object url){
        ...
        if(url instanceof String)return RegexUtilities.WEB_URL.matcher(String.valueOf(url)).matches();
    ...
    public static boolean isNumeric(Object str) {
        ...
        if(str instanceof String){
            ...
            return RegexUtilities.IS_NUMERIC.matcher(str2).matches();
        }
    ...
    public static boolean isInt(Object str) {
        if(str instanceof String) return RegexUtilities.IS_INT.matcher(String.valueOf(str)).matches();
    ...
    public static boolean isDouble(Object str) {
        if(str instanceof String)return RegexUtilities.IS_DOUBLE.matcher(String.valueOf(str)).matches();
    ...
    public static boolean isFloat(Object str) {
        if(str instanceof String)return RegexUtilities .IS_FLOAT.matcher(String.valueOf(str)).matches();
    ...
    public static boolean isDecimal(Object str) {
       /* try {Float.parseFloat(str);return true;
        ...
        if(str instanceof String) return RegexUtilities.IS_DECIMAL.matcher(String.valueOf(str)).matches();
    ...
    public static boolean isLineSeparator(String str){
        return RegexUtilities.IS_LINE_SEPARATOR().matcher(str).matches();
    }
  ...
  /*  public static String convertByteArrayToStringWithApacheCommonCodec(byte[] arrayBytes){
    ...
    @Deprecated
    public static String deleteSpecialChar(final String badFileName) {
    	int[] a;
    	Arrays.sort(a = new int[] { 42, 64, 92, 47, 40, 41, 91, 93, 63, 60, 62, 33, 123, 125, 58, 34, 124 });
        final StringBuilder sb = new StringBuilder();
        for (int i = 0; i < badFileName.length(); ++i) {
            final char char1 = badFileName.charAt(i);
            if (Arrays.binarySearch(a, char1) < 0) {
                sb.append(char1);
            }
        }
        return sb.toString();
    ...
    public static String formatFileDimension(final long bytes) {
        final int n = 1024;
        if (bytes < n) {
            return bytes + " B";
        }
        final int n2 = (int)(Math.log(bytes) / Math.log(n));
        return String.format("%.1f %sB", bytes / Math.pow(n, n2), "KMGTPE".charAt(n2 - 1) + "");
    }
    
    public static String getNumberRangeString(final Integer number, final Integer rangeLength) {
    	int[] a;
    	Arrays.sort(a = new int[] { 42, 64, 92, 47, 40, 41, 91, 93, 63, 60, 62, 33, 123, 125, 58, 34, 124 });
    	final IntRange a2 = getNumberRange(number, rangeLength);
        return String.format("%s-%s", a2.getMinimumInteger(), a2.getMaximumInteger());
    }
    
    private static IntRange getNumberRange(final Integer n, final Integer n2) {
        final int n3 = (n - 1) / n2;
        return new IntRange(n3 * n2 + 1, n3 * n2 + n2);
    }
